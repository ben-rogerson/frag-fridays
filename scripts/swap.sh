...
#
# Usage: scripts/swap.sh <vanilla|gg|dm|aim|css|fy|awp>
set -euo pipefail
cd "$(dirname "$0")/.."
HOST="${CS16_HOST:-cs16}"
...
case "$mod" in
  ...
  fy)      label="Fight Yard" ;;
  awp)     label="Sniper" ;;
  *) echo "usage: scripts/swap.sh <vanilla|gg|dm|aim|css|fy|awp>" >&2; exit 1 ;;
esac

# Heads-up to connected players. Only the mod-dir builds have cmdpipe - if vanilla
