...
One breakpoint at 760px: the grid collapses to a single column reordered servers → card → standings → aside, the session strip stacks and its clock digits shrink, the masthead online badge disappears, the server table drops its round/map-time columns (columns 4+), and the standings drop their sessions/time/mvp columns (the weekly and practice tables drop time) so the essentials fit a phone.
