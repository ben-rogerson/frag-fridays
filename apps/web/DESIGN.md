...
One breakpoint at 760px: the grid collapses to a single column reordered servers → card → standings → aside, the session strip stacks and its clock digits shrink, the masthead online badge disappears, the server table drops its round/map-time columns (columns 4+), and the standings drop their sessions/time/mvp columns (the weekly and practice tables drop plants and time) so the essentials fit a phone.
...
- **Secondary:** small bevel chrome buttons - navy gradient (#2c4278 → #1d2f5c) or translucent navy, Phosphor White text at 0.62rem uppercase, hover brightens via filter, same Hot Acid focus outline.
...
### Session Clock, in-game (signature)
Where the Friday session is up to, over the running game, on screen only for
as long as **Tab** is held: a bug hanging off the top edge of the screen,
centred - solid Ink Deep, bevel border, no top edge, the mode's accent on
tabular digits led by a 0.68rem tracked label. It rides the scoreboard key
because that is where a player already looks for match state, which is also
why it is solid and full strength rather than shy: nothing competes for the
eye at the moment it appears, and the rest of the time the screen is the
game's. It says one of two things and otherwise nothing at all: mid-session
the slot's remaining time ("SESSION 12:04 LEFT"), and on matchday before
kickoff the wait for it ("SESSION IN 3:41:20", hours dropping off inside the
hour). Any other day it does not appear - a Tuesday warm-up does not need a
three-day countdown over the game. Two escalations, reading the same whether
they are the end of the session or the wait for its start: the last five
minutes put the digits and border in Kill-Feed Red, and the final minute
blinks the digits on the same one-second beat as the countdown's colons. It
lives outside the overlay because the page is hidden while playing, and never
takes the pointer - a click there has to reach the canvas and re-lock the
mouse.
...
The SEASON STANDINGS panel closes the main column: tournament coverage for a six-person server, played completely straight. The tables share the server browser's grammar (micro-label headers on a faint wash, 0.8rem cells, blue hover wash, Bevel Highlight row dividers) with two additions: numeric columns are right-aligned, and every figure is tabular. Season columns run rank / player / sessions / k-d ("88 / 17", one cell) / k-d % (kills as a share of kills + deaths, "84%") / plants (bombs planted that detonated) / time / mvps; time renders coarse ("47m", "2h 05m"), empty plants and mvp cells read "-". Rank digits sit in Faint Slate; the top three wear 🥇🥈🥉 in place of the digit. Below the table a footer line in Faint Slate states the session count and last MVP; with no sessions yet the panel holds a single flat line ("no ranked results yet - the table publishes after the first friday session"). Then the week-by-week breakdown, newest first, every week folded closed so the season table keeps the stage: one flush sub-bar per Friday since the first session ("week 3 · fri 21 aug", bar note "mvp ZenBot (102 kills)") acts as the disclosure (a "+" / "-" glyph leads it) over a short rank / player / k-d / k-d % / plants / time table with the same medals. A Friday with nothing in the logs keeps its sub-bar with the note "no stats recorded" and no table - the gap is stated, never hidden - and a week with incomplete logs carries a flat footer line saying what is missing. During the practice period a flush PRACTICE STANDINGS sub-bar (bar note "warm-up frags - reset at kickoff", dated once a session exists) adds a second, shorter table - rank / player / k-d / k-d % / plants / time, digits only, no medals; it sits out entirely while the strip reads LIVE. The panel renders only once `/assets/standings.json` has answered - the page never invents results - and both tables rank humans only.
