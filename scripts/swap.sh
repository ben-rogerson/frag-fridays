...
#
# Usage: scripts/swap.sh <vanilla|gg|dm|aim>
set -euo pipefail
cd "$(dirname "$0")/.."
HOST="${CS16_HOST:-cs16}"
...
case "$mod" in
  ...
  dm)      label="Deathmatch" ;;
  aim)     label="Aim Prac" ;;
  *) echo "usage: scripts/swap.sh <vanilla|gg|dm|aim>" >&2; exit 1 ;;
esac

# Heads-up to connected players. Only gg/dm/aim have cmdpipe - if vanilla
